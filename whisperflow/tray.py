...
def start_progress_indicator(
    ...
    *,
    ...
    """Start a tray indicator that updates the tooltip from a queue."""
    icon_value, icon_is_path = resolve_tray_icon(icon_name)
    runner = _load_progress_runner()
    if runner is None:
        logger.warning(
            "Progress tray indicator requested but no supported tray backend is available."
        )
        return None
    thread = threading.Thread(
        target=runner,
        args=(stop_event, tooltip, icon_value, icon_is_path, progress_queue),
        name="whisperflow-tray-progress",
        daemon=True,
    )
    thread.start()
    return thread
...
def resolve_tray_icon(icon_name: str) -> tuple[str, bool]:
    ...
    if icon_name == "retranscribe":
        return _ensure_processing_icon("#2563eb", "whisperflow-retranscribe.svg"), True
    if icon_name == "mix":
        return _ensure_processing_icon("#16a34a", "whisperflow-mix.svg"), True
...
def _ensure_processing_icon(color: str, filename: str) -> str:
    icon_path = Path(tempfile.gettempdir()) / filename
    if icon_path.exists():
        return str(icon_path)
    svg = (
        "<svg xmlns='http://www.w3.org/2000/svg' width='64' height='64'>"
        "<circle cx='32' cy='32' r='20' fill='"
        + color
        + "'/>"
        "</svg>"
    )
...
def _load_progress_runner(
) -> Callable[[threading.Event, str, str, bool, queue.Queue[str]], None] | None:
    runner = _load_appindicator_progress_runner()
    if runner is not None:
        return runner
    runner = _load_gtk_progress_runner()
    if runner is not None:
        return runner
    return _load_pystray_progress_runner()
...
def _load_appindicator_progress_runner(
) -> Callable[[threading.Event, str, str, bool, queue.Queue[str]], None] | None:
    try:
        import gi  # type: ignore
    ...
    def run(
        ...
        try:
            ...
            menu = Gtk.Menu()
            item = Gtk.MenuItem(label=tooltip)
            item.set_sensitive(False)
            menu.append(item)
            menu.show_all()
            indicator.set_menu(menu)
            ...
            def check_updates() -> bool:
                ...
                while True:
                    try:
                        tooltip_text = progress_queue.get_nowait()
                    except queue.Empty:
                        break
                    item.set_label(tooltip_text)
                    if hasattr(indicator, "set_title"):
                        indicator.set_title(tooltip_text)
                    if hasattr(indicator, "set_icon_full"):
                        indicator.set_icon_full(icon_value, tooltip_text)
                    updated = True
                if updated:
                    menu.show_all()
                return True

            GLib.timeout_add(250, check_updates)
            loop.run()
        except Exception as exc:  # noqa: BLE001
            logger.warning("Tray progress indicator failed to start: %s", exc)
...
def _load_pystray_progress_runner(
) -> Callable[[threading.Event, str, str, bool, queue.Queue[str]], None] | None:
    try:
        import pystray  # type: ignore
        from PIL import Image, ImageDraw  # type: ignore
    ...
    def run(
        ...
        icon = pystray.Icon("whisperflow-progress", _build_icon((37, 99, 235)), tooltip)
        ...
        def update_tooltip() -> None:
            while not stop_event.is_set():
                try:
                    tooltip_text = progress_queue.get(timeout=0.5)
                except queue.Empty:
                    continue
...
__all__ = ["start_tray_indicator", "start_progress_indicator"]
